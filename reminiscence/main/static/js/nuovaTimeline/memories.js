...
function filterListener(string){
filterBoolean[string]=!filterBoolean[string];
controlFilters();
}

function isFilteredIn(a){
	
	var s=(a.personal?"private":"public");
	return filterBoolean[a.type]&&filterBoolean[s];
	}
	
	
function controlFilters(){
	for (var w in associativeArray){//Restore presence and set scale
		var tmp=associativeArray[w];
		if(!isFilteredIn(tmp)){
			tmp.div.style.visibility="hidden";
			tmp.placeHolder.style.visibility="hidden";
			tmp.span.style.visibility="hidden";
			tmp.placeHolderSpan.style.visibility="hidden";
			}else{
				if(tmp.drawed){
					tmp.div.style.visibility="visible";
					tmp.span.style.visibility="visible";
					}else{
					tmp.placeHolder.style.visibility="visible";
					}
				}
		}
...
var url = "http://devfever.altervista.org/";
var extension = ".jpg";
var size;
var sizedef;
var sizedefHalf;
...
$(document).ready(function (e) {
   ...
   // $('#timelinePage').css(getGradientColor(BGCOLOR1, BGCOLOR2)); //Color body
...
Timeline.prototype.generateYearSigns = function () {
		...
		line.style.left = a.getYearPos(a.roundedFirstYear);
		line.style.width =(a.getYearPos(a.endYear)- a.getYearPos(a.roundedFirstYear))+ "px";
    ...
    createMemories(pos2);
	$("#infoYear").html(a.getPosYear(dePixel(timeline.style.left)));
}
...
function createYearSign(year, distance) {
	...
	newspan.style.textShadow=YEARSIGNSTEXTSHADOW;
...
function zoom(shamt, register) {
    ...
    if ((globalScale * shamt >= 1) && (globalScale * shamt < globalScaleLimit)) {
        globalScale *= shamt;
		var scale=Math.round(globalScale*10)/10;
		$("#infoScale").html(scale+"x");
...
var memory = function (nominalYear, actualYear, src, actualPos, placeHolder, title, text, type, index, personal, photoNumber) {
    ...
    this.placeHolderSpan = document.createElement('span');
	   this.placeHolderSpan.style.textShadow=YEARSIGNSTEXTSHADOW;
...
memory.prototype.setVisible = function (boolean) {
    ...
    if (boolean) {
		var filterPassed=(isFilteredIn(this));
		if(filterPassed){
this.div.style.visibility = "visible";}
$( this.placeHolder).addClass("icon");
$( this.placeHolder).removeClass("rhombus");
	...
	if (middleLine == this.placeHolder.style.top) {
		...
		$( this.placeHolder).removeClass("icon");
		$( this.placeHolder).addClass("rhombus");
        if (middleLine != this.placeHolder.style.top) {
            if (this.upper) {
